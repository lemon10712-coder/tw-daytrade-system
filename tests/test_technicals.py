...
def test_volume_expansion_ratio_nan_when_base_is_exactly_zero():
    base = np.zeros(20)
    spike = np.concatenate([base, np.full(5, 5.0)])
    assert np.isnan(volume_expansion_ratio(spike, recent_window=5, base_window=20))


def test_volume_expansion_ratio_nan_when_base_near_zero_but_nonzero():
    """2026-09-07 真實案例：6649 台生材基準期均量只有 0.0x 張（幾乎沒有交易），
    近5日均量 7.8 張，若不設門檻會算出 3907 倍這種離譜比率，汙染族群強度排名
    （見 KNOWN_ISSUES.md 診斷紀錄）。基準期均量低於 1 張時應視為資料不足，回傳 nan，
    而不是算出一個看起來很精確、實際上只是雜訊被除大的數字。"""
    base = np.full(20, 0.05)  # 基準期均量 0.05 張，遠低於 1 張的門檻，但不是恰好 0
    spike = np.concatenate([base, np.full(5, 7.8)])
    ratio = volume_expansion_ratio(spike, recent_window=5, base_window=20)
